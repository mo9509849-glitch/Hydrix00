...
const state = {
  ...
  // ---- مكتشف الأعطال ----
  pumpStartMoisture: null,   // رطوبة التربة لحظة تشغيل المضخة
  pumpSawRain: false,        // هل رُصد مطر أثناء دورة الري الحالية؟
  pumpAnomalyChecked: false, // هل تم فحص عطل المضخة في الدورة الحالية أثناء التشغيل؟
  dryAlerted: false,         // لمنع تكرار تنبيه "التربة جافة" كل ثانية
  postPumpCheck: null,       // مراقبة تسريب المياه بعد إيقاف المضخة
  rainTrueStreak: 0,         // عدد العينات المتتالية اللي حساس المطر قال فيها "فيه مطر"
  lastAnomaly: JSON.parse(localStorage.getItem("hydrix_lastAnomaly") ?? "{}"),
};
...
/* ---------------- توصيات رطوبة التربة حسب المحصول (تقريبية) ---------------- */
const CROP_THRESHOLDS = {
  ...
  "نعناع":       { on: 55, off: 80 },
  "بطيخ":        { on: 40, off: 65 },
  "نباتات زينة": { on: 35, off: 60 },
};

function applyCropThresholds(crop, { silent = false } = {}) {
  const rec = CROP_THRESHOLDS[crop];
  if (!rec) return false;
  state.thresholds = { on: rec.on, off: rec.off };
  localStorage.setItem("hydrix_th", JSON.stringify(state.thresholds));
  syncThresholdUI();
  const sent = sendBLE(`TH:${rec.on},${rec.off}`);
  if (!silent) {
    toast(
      sent
        ? `المحصول: ${crop} — تم ضبط حدود الري (${rec.on}% - ${rec.off}%) وإرسالها للجهاز`
        : `المحصول: ${crop} — تم ضبط حدود الري الموصى بها (${rec.on}% - ${rec.off}%)`
    );
  }
  return true;
}
...
/* ---------------- الراوتر ---------------- */
const SCREENS = ["welcome", "crop-select", "dashboard", "manual", "timed", "smart", "plant", "camera", "history", "settings", "about"];
...
$("btnStart").addEventListener("click", () => {
  // أول مرة (أو لو المزارع لسه ما حددش محصول) هنعدّي بيه على شاشة اختيار المحصول الأول
  location.hash = state.crop ? "dashboard" : "crop-select";
});

// اختيار المحصول أول مرة (بعد شاشة الترحيب مباشرة)
document.querySelectorAll("#cropGridFirst .crop-chip-big").forEach((chip) =>
  chip.addEventListener("click", () => {
    state.crop = chip.dataset.crop;
    localStorage.setItem("hydrix_crop", state.crop);
    applyCropThresholds(state.crop);
    location.hash = "dashboard";
  })
);
$("btnCropSkip").addEventListener("click", () => { location.hash = "dashboard"; });
...
/* ================= مكتشف الأعطال الذكي ================= */
function raiseIssue(type, message) {
  const now = Date.now();
  const last = state.lastAnomaly[type] || 0;
  if (now - last < 3600000) return; // مرة كل ساعة بالكتير لكل نوع عطل
  state.lastAnomaly[type] = now;
  localStorage.setItem("hydrix_lastAnomaly", JSON.stringify(state.lastAnomaly));
  console.error("[Hydrix تشخيص]", message);
  $("errorBannerText").textContent = message;
  $("errorBanner").classList.remove("hidden");
  notify("⚠️ " + message);
  toast("⚠️ " + message, 5000);
}
$("errorBannerClose").addEventListener("click", () => $("errorBanner").classList.add("hidden"));

// 1) عطل في المضخة أو انسداد — أي مدة تشغيل حتى لو ثواني، لأن الحساس قريب من الخرطوم
function checkPumpMoistureAnomaly(dur) {
  if (state.pumpStartMoisture == null || !dur) { state.pumpStartMoisture = null; return; }
  const rise = state.moisture - state.pumpStartMoisture;
  if (rise < 3 && !state.pumpSawRain && !state.pumpAnomalyChecked) {
    raiseIssue("pump", "ممكن يكون في عطل في المضخة أو انسداد في الخراطيم — الري اشتغل ومع ذلك رطوبة التربة ما ارتفعتش.");
  }
  // بداية مراقبة التسريب بعد الإيقاف
  state.postPumpCheck = { ts: Date.now(), startMoisture: state.moisture, rainSeen: state.rain };
  state.pumpStartMoisture = null;
  state.pumpAnomalyChecked = false;
}

// 1-ب) نفس الفحص لكن أثناء تشغيل المضخة نفسها — من غير ما تستنى تقفلها
// بيتنادى كل ثانية من الحلقة الرئيسية طول ما المضخة شغالة
function checkPumpRunningAnomaly() {
  if (!state.pumpOn || state.pumpAnomalyChecked || state.pumpStartMoisture == null) return;
  const elapsed = Date.now() - state.pumpStartTs;
  if (elapsed < 5000) return; // ننتظر 5 ثواني من بداية التشغيل
  const rise = state.moisture - state.pumpStartMoisture;
  if (rise < 1.5 && !state.pumpSawRain) {
    raiseIssue("pump", "ممكن يكون في عطل في المضخة أو انسداد في الخراطيم — مر 5 ثواني على تشغيل الري ورطوبة التربة ما اتحركتش.");
  }
  state.pumpAnomalyChecked = true; // منفحصش تاني في نفس دورة التشغيل دي
}

// 2) تسريب مياه بعد إيقاف المضخة (الرطوبة استمرت ترتفع لوحدها من غير مطر)
function checkLeakAnomaly() {
  if (!state.postPumpCheck) return;
  if (state.pumpOn) { state.postPumpCheck = null; return; }
  if (state.rain) state.postPumpCheck.rainSeen = true;
  const elapsed = Date.now() - state.postPumpCheck.ts;
  if (elapsed < 300000) return; // ننتظر 5 دقايق
  const rise = state.moisture - state.postPumpCheck.startMoisture;
  if (rise >= 5 && !state.postPumpCheck.rainSeen) {
    raiseIssue("leak", "ممكن يكون في تسريب مياه — المضخة مقفولة من فترة ومع ذلك رطوبة التربة استمرت في الارتفاع من غير مطر.");
  }
  state.postPumpCheck = null;
}

// 3) حساس المطر عالق (بيفضل يقول "فيه مطر" رغم إن الأرصاد بتقول لأ)
function checkRainStuckAnomaly() {
  state.rainTrueStreak = state.rain ? state.rainTrueStreak + 1 : 0;
  const forecastLow = state.rainForecast !== null && state.rainForecast < 10;
  if (state.rainTrueStreak >= 6 && forecastLow) { // 6 عينات × 5 دقايق ≈ نص ساعة
    raiseIssue("rainStuck", "ممكن يكون حساس المطر عالق — بيرصد مطر مستمر من نص ساعة بينما توقعات الأرصاد بتقول مفيش مطر.");
    state.rainTrueStreak = 0;
  }
}

// 4) التحقق التبادلي: حساس المطر × رطوبة الجو × رطوبة التربة
function checkRainMoistureCombined() {
  const recent = state.samples.slice(-6); // آخر نص ساعة تقريباً
  if (recent.length < 6) return;
  const pumpWasOffWhole = recent.every((s) => !s.p);
  if (!pumpWasOffWhole) return; // لازم نعزل تأثير المطر لوحده عن تأثير المضخة

  const rainFlagged = recent.some((s) => s.r);
  const humOld = recent[0].h;
  const humNew = recent[recent.length - 1].h;
  const humRise = (humNew - humOld) >= 20;                                  // ارتفاع ملحوظ: 20 نقطة مئوية زيادة
  const soilRise = (recent[recent.length - 1].m - recent[0].m) >= 3;

  if (rainFlagged && humRise && !soilRise) {
    raiseIssue("soilVsRain", "على الأغلب حساس رطوبة التربة فيه عطل — حساس المطر ورطوبة الجو بيأكدوا إن فيه مطر فعلي، ومع ذلك رطوبة التربة ما اتغيرتش.");
  } else if (rainFlagged && !humRise && !soilRise) {
    raiseIssue("rainFalsePositive", "ممكن يكون حساس المطر بيدي قراءة غلط — مفيش أي مؤشر تاني (رطوبة الجو أو رطوبة التربة) بيأكد إن فيه مطر فعلاً.");
  } else if (!rainFlagged && humRise && !soilRise) {
    raiseIssue("rainMissed", "ممكن يكون حساس المطر مش بيرصد مطر فعلي — رطوبة الجو ارتفعت بشكل ملحوظ (20% فأكتر) وده مؤشر على مطر محتمل، لكن الحساس ما سجّلش حاجة.");
  }
}

// 5) حساس الرطوبة متجمد (نفس القراءة بالظبط لفترة طويلة)
function checkMoistureStuckAnomaly() {
  const recent = state.samples.slice(-6);
  if (recent.length < 6) return;
  const vals = recent.map((s) => s.m);
  const spread = Math.max(...vals) - Math.min(...vals);
  if (spread < 0.5) {
    raiseIssue("moistureStuck", "ممكن يكون حساس الرطوبة متجمد أو واقع — القراءة ما اتغيرتش خالص من نص ساعة.");
  }
}
...
function setPump(on, { quiet = false, source = "manual", auto = false, ble = true } = {}) {
  ...
  if (on) {
    ...
    state.pumpStartMoisture = state.moisture;
    state.pumpSawRain = state.rain;
    state.pumpAnomalyChecked = false;
    ...
    else notify(`بدأ الري (${MODE_NAMES[source] || source}) — رطوبة التربة ${Math.round(state.moisture)}%`);
  } else {
    const dur = state.pumpStartTs ? Date.now() - state.pumpStartTs : null;
    checkPumpMoistureAnomaly(dur);
    ...
    else notify(`توقف الري (${MODE_NAMES[source] || source}) — المدة ${Math.round((dur || 0) / 60000)} دقيقة`);
  }
...
HydrixBLE.onMessage = (data) => {
  ...
  if (data.rain !== undefined) {
    state.rain = !!Number(data.rain);
    if (state.pumpOn && state.rain) state.pumpSawRain = true;
  }
  ...
  if (data.pump !== undefined) {
    ...
    if (on !== state.pumpOn) {
      ...
      if (on) {
        ...
        else notify(`بدأ الري (من الجهاز) — رطوبة التربة ${Math.round(state.moisture)}%`);
      } else {
        const dur = state.pumpStartTs ? Date.now() - state.pumpStartTs : null;
        checkPumpMoistureAnomaly(dur);
        state.lastStopTs = Date.now();
        ...
        else notify(`توقف الري (من الجهاز) — المدة ${Math.round((dur || 0) / 60000)} دقيقة`);
      }
    }
  }
  render();
...
setInterval(() => {
  ...
  // تنبيه "التربة جافة" — يعتمد على حد الري الخاص بالمحصول المختار (edge-triggered، مرة واحدة لحد ما ترتفع الرطوبة تاني)
  if (state.moisture < state.thresholds.on && !state.dryAlerted) {
    state.dryAlerted = true;
    notify(`🌱 التربة جافة (${Math.round(state.moisture)}%) — هيبدأ الري لو الري الذكي مفعّل، أو شغّله يدوياً.`);
  } else if (state.moisture >= state.thresholds.off) {
    state.dryAlerted = false; // إعادة تفعيل التنبيه لما تجف التربة تاني في المرة الجاية
  }

  // تنبيه انخفاض الرطوبة الخطير جداً (مرة كل ساعة كحد أقصى)
  if (state.moisture < 20 && now - state.lastLowAlert > 3600000) {
    state.lastLowAlert = now;
    notify(`⚠️ رطوبة التربة منخفضة جداً (${Math.round(state.moisture)}%) — النبات عطشان!`);
  }

  // فحص عطل المضخة أثناء التشغيل نفسه (بعد 5 ثواني من بدء الري)
  checkPumpRunningAnomaly();

  // تسجيل عينة رطوبة كل 5 دقائق للرسم البياني + بيانات مكتشف الأعطال
  if (now - state.lastSampleTs > 300000) {
    state.lastSampleTs = now;
    state.samples.push({
      t: now,
      m: Math.round(state.moisture),
      p: state.pumpOn ? 1 : 0,
      r: state.rain ? 1 : 0,
      h: Math.round(state.hum),
    });
    state.samples = state.samples.slice(-288); // 24 ساعة
    localStorage.setItem("hydrix_samples", JSON.stringify(state.samples));

    checkRainStuckAnomaly();
    checkRainMoistureCombined();
    if (HydrixBLE.isConnected()) checkMoistureStuckAnomaly();
  }

  checkLeakAnomaly(); // يتفحص كل ثانية عشان يلتقط التسريب بدقة
...
document.querySelectorAll(".crop-chip").forEach((chip) =>
  chip.addEventListener("click", () => {
    ...
    localStorage.setItem("hydrix_crop", state.crop);
    syncSettingsUI();
    const applied = state.crop && applyCropThresholds(state.crop);
    if (!applied) toast(state.crop ? `المحصول: ${state.crop} — سيُستخدم في تشخيص الـ AI` : "تم إلغاء تحديد المحصول");
    render();
  })
);
